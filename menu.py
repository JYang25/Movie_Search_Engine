        ...
        if choice == '1':
            while (query != 'q'):
                ...
                for m in result_list:
                    print(f' -  [{m[1]}] | Director: {m[3]} | Star: {m[2]} | Recommend Rate(0-5): {m[5]}')
        elif choice == '2':
            recommend_list = recommTop20(user_id)
            for m in recommend_list:
                if str(m[0]) in M:
                    print(f' -  [{M[str(m[0])].GetTitle()}] | Director: {M[str(m[0])].GetDirector()} | Star: {M[str(m[0])].GetStar()} | Recommend Rate(0-5): {m[1]}')
        elif choice == '3':
            genres_menu()
        elif choice == '4':
